...
export const getUserGroupPostsByStatus = (userId, groupId, status, offset, callback) => {
   const limit = 3; // Số lượng bài viết trên mỗi trang
   groupModel.getUserPostsByGroupIdAndStatus(userId, groupId, status, offset, limit, (err, data) => {
      if (err) return callback(err, null);
      return callback(null, data);
   });
};