...
export const changeGroupLeader = async (req, res) => {
   try {
      const currentLeaderId = await AuthService.verifyUserToken(req.cookies.accessToken);

      AuthService.IsAccountBanned(currentLeaderId, async (err, data) => {
         if (err) {
            return res.status(403).json({ message: "Your account is banned or invalid." });
         }

         const { newLeaderId, groupId } = req.body;

         joinService.changeGroupLeader(currentLeaderId, newLeaderId, groupId, (err, response) => {
            if (err) return res.status(500).json({ error: err.message });
            return res.json({ message: "Group leader changed successfully." });
         });
      });
   } catch (error) {
      return res.status(500).json({ error: error.message });
