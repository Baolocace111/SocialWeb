   ...
   db.query(
      ...
      (error, results) => {
         ...
         return callback(
            null,
            results.map((post) => {
               const videoExtensions = ['.mp4', '.avi', '.mov']; // Danh sách các phần mở rộng video
               const imgExtensions = ['.jpg', '.jpeg', '.png', '.gif']; // Danh sách các phần mở rộng hình ảnh

               const isVideo = videoExtensions.some(ext => post.img.endsWith(ext));
               const isImage = imgExtensions.some(ext => post.img.endsWith(ext));

               return {
                  id: post.id,
                  type: isVideo ? 'video' : isImage ? 'image' : 'unknown', // Xử lý trường hợp không xác định
               };
            })
         );
      }
  ...
  DELETE posts.* FROM posts WHERE posts.id = ?;`;
   ...
   db.query(deleteQuery, [postId, postId], (err, deletedPost) => {
      ...
      if (deletedPost[1].affectedRows > 0) {
         ...
         return callback(null, "Post is undefined");
      }
   });
};
